...
ITEMS = [Items('Alexei', 'Kaput', '1945'), Items(),\
    Items('panther', "the most beautiful panther eva", 400, "shitty_panther_ytsv-p3.png"), Items(), Items(), Items(), Items(), Items(), Items(), Items('Кактус', "Хороший кактус", 600, "ball.jpg")]
FEEDBACK = []
...
def about():
    ...
    resp = make_response(render_template('about.html', feedback=FEEDBACK, title="test "))
    if login:
        resp.set_cookie('menu', login)
    return resp
...
def catalog():
    ...
    return render_template('catalog.html', items=items, title="Каталог")
